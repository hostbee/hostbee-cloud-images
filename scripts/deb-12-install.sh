...
if [ "$CN_FLAG" == "true" ]; then
    echo "use CN sources"
    sudo sed -i 's/deb.debian.org/mirrors.cernet.edu.cn/g' /etc/apt/sources.list.d/debian.sources

    sudo sed -i 's/deb.debian.org/mirrors.cernet.edu.cn/g' /etc/apt/mirrors/debian.list
