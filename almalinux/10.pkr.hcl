...
source "qemu" "almalinux" {
  ...
  disk_size                 = "10G"
  headless                  = true
  iso_checksum              = "file:https://mirror.nju.edu.cn/almalinux/10/cloud/x86_64/images/CHECKSUM"
  iso_url                   = "https://mirror.nju.edu.cn/almalinux/10/cloud/x86_64/images/AlmaLinux-10-GenericCloud-latest.x86_64.qcow2"
  output_directory          = "${var.cn_flag == "true" ? "output-almalinux-10-cn" : "output-almalinux-10"}"
  shutdown_command          = "sudo -S bash /root/cleanup.sh"
  shutdown_timeout          = "15s"
  ssh_username              = "builder"
  ssh_private_key_file      = local.ssh_private_key_file
  ssh_clear_authorized_keys = true
  vm_name                   = "almalinux-10.img"
  cpu_model                 = "host"
...
build {
  ...
  provisioner "shell" {
    ...
    scripts = [
      "scripts/almalinux-10-install.sh",
