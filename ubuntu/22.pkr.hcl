...
source "qemu" "ubuntu" {
  ...
  disk_size                 = "10G"
  headless                  = true
  iso_checksum              = "file:https://mirrors.cernet.edu.cn/ubuntu-cloud-images/${var.ubuntu_version}/current/SHA256SUMS"
  iso_url                   = "https://mirrors.cernet.edu.cn/ubuntu-cloud-images/${var.ubuntu_version}/current/${var.ubuntu_version}-server-cloudimg-amd64.img"
  output_directory          = "${var.cn_flag == "true" ? "output-ubuntu-22-cn" : "output-ubuntu-22"}"
  shutdown_command          = "sudo -S shutdown -P now"
  ssh_username              = "builder"
  ssh_private_key_file      = local.ssh_private_key_file
  ssh_clear_authorized_keys = true
  vm_name                   = "ubuntu-22.img"
